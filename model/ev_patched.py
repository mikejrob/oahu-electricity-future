"""Electric-vehicle charging module — MODIFIED COPY of Switch's
switch_model/hawaii/ev.py.

Copyright the Switch Authors (Matthias Fripp and the Switch-Hawaiʻi
contributors); modifications copyright 2026 Michael J. Roberts and
contributors. Licensed under the Apache License, Version 2.0; see the
LICENSE and NOTICE files at the repository root.

Modification (the only change from upstream): adds the derived parameter
`ev_mwh_ts`, the business-as-usual EV charging energy aggregated to each
timeseries, which this study's inputs require. Upstream defines EV demand
only per timepoint. The module is loaded in place of
switch_model.hawaii.ev by build/build_corrected_inputs.py.
"""
