...
def define_arguments(argparser):
    ...
    argparser.add_argument(
        ...
        type=int,
        default=None,
        help="Earliest period in which LNG infrastructure can be activated. "
             "E.g., --lng-earliest-period 2030 blocks LNG before 2030.",
    )
...
def define_components(m):
    ...
    # Block LNG before earliest allowed period (FSRU infrastructure lead time)
    if m.options.lng_earliest_period is not None:
        earliest = m.options.lng_earliest_period

        def Block_Early_LNG_rule(m, rfm, per, tier):
            if per < earliest:
                return m.RFMSupplyTierActivate[rfm, per, tier] == 0
            return Constraint.Skip

        m.Block_Early_LNG = Constraint(
            m.LNG_RFM_SUPPLY_TIERS, rule=Block_Early_LNG_rule
        )
        if m.options.verbose:
            print(f"Blocking LNG infrastructure before period {earliest}.")
