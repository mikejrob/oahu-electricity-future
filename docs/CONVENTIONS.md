...
the EIA AEO 2025 series), deflated to 2024$. The other three oil paths are
market-derived (`build/market_band/`): the Brent futures strip as the central
market case and the 10th/90th percentiles from the implied volatility of
Brent options, converted to real dollars with TIPS breakevens and held flat
in real terms beyond the last liquid contracts (`sources/market/METHOD.md`;
report Appendix A.14). `apply_market_band.py` shifts each input directory's
fuel curves off the reference using the published regressions from Roberts
(2026), `hawaii-lng-lsfo-brief`:
...
The earlier AEO2025 case spread (Reference $91 / Low $48 / High $157/bbl at
2050) built by `build_brent_variants.py` is retired: those inputs and solves
are archived (`*_aeo.csv`, `aeo_archive/`) and the script is kept for
provenance only.
