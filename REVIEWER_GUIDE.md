...
| 3 | **Battery co-location** | Derived from NREL's own PV-plus-battery hybrid: `(PVB−PV)/0.5` — interconnection saved + NREL's joint-install delta (~0.91–0.93 by year) | Recompute from `sources/ATBe_2024_v3.0.0_slice.csv` (PVB Class5 rows); compare the old flat 0.88 (2-hr GCC share) in `docs/CORRECTIONS.md` |
