...
Every scenario in this analysis, in plain language. Each row is one solved
optimization (at 0.25% tolerance, refined to 0.1%); machine-readable
...
Oil paths: `lowbrent` / `refbrent` / `highbrent` (EIA AEO 2025 case spread).
`_j120` = JERA capital at the vendor's +20% case. `_adv` = ATB *Advanced*
solar+battery basis. `lc_` = Class-C-only land screen.

## 1. The headline matrix (6 trajectories × 3 oil paths, 2 land screens)
...
| `C6_STATUSQUO` | Waiau + JERA LNG 500 — the full announced program. |

All of the above solve at three oil paths on the reference land screen —
agricultural/country-zoned land, Class A/golf/roads/slopes>10% excluded,
prime B/C capped at 10% per cluster, all D/E and non-ag admitted (27,256
eligible acres; the current-law "S0" envelope in the companion land study,
