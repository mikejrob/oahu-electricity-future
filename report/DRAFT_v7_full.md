...
date: "pre-v1.032 DRAFT — September 2026 · open for comment"
note: "This is a preliminary release, pre-v1.032, open for public comment; we ask that comments and suggestions arrive by September 15, 2026 (tentative; the repository README carries the current date). After the comment period and our responses, the report will be locked as version 1, and further suggestions will be directed to the v2 regional-grid model. This report supersedes the authors' 2026 working paper; the changes relative to it are documented in docs/CORRECTIONS.md. All dollar figures are real 2024 US$, present value as of 2027. Scenario
  results are at 0.1% optimization tolerance; a handful of hard cells
  stopped marginally above it at their time limit, and
  results/RESULTS_SUMMARY.csv records each cell's requested and achieved
  gap (docs/HARD_CELLS.md)."
---

## Executive Summary

This report asks what Hawaiian Electric and Hawaiʻi should build to keep
Oʻahu's lights on through 2050 as the grid moves to 100 percent clean
power, and what it will cost. We solve a capacity-expansion model, the
planning tool utilities and regulators use to schedule decades of
construction, several hundred times. Each run varies oil prices, solar and
battery costs, land-use rules, the Waiau Repower, and the JERA LNG
proposal. Every input traces to a public source or a labelled author
assumption, and the full model, data, and code are public. The base case
follows current federal law, including the storage and geothermal tax
credits (48E) that survive the 2025 budget reconciliation act; we report a
no-credit sensitivity for the case those credits are denied or repealed.
Five findings organize the results. The two largest levers are both about
how Hawaiʻi builds solar — on open land and on rooftops. The choices that
dominate public debate matter less.

**Finding 1. The largest lever is the price Hawaiʻi pays to build solar and
batteries.** Solar delivered here costs far more than other states. The
...
billion; at 70 percent above, by about $2.9 billion. Our baseline already
includes a 20 percent Hawaiʻi premium, so these cases correspond to roughly
1.8 and 2.0 times the national benchmark, near what recent procurement
...
without the clean-energy mandate, because even at twice the national average 
cost, solar will be the cheapest energy source. Reforms that close even part 
of the gap are worth more than any fuel decision in this report, and they are
within the State's own control (Section 2).
...
over 2020–2024. In 2024 the installed base reached the level Hawaiian
Electric's 2016 plan projected for 2034 (Appendix A.13). We measure what
these distributed resources do to the grid directly from utility demand records: 
each installed megawatt removes about 0.61 MW of midday grid demand, and home 
batteries shift about 45 percent of their storage capacity into the evening 
hours each day (Appendix A.11). Because each megawatt of rooftop capacity 
displaces about 0.7 MW of utility-scale solar, rooftop growth determines how 
much open land is needed for grid-scale solar. Limited further growth of rooftop solar leads to 
about 4,200 MW of utility-scale solar on about 20,900 acres by 2050; continuing
the current trend requires only about 3,800 MW on about 19,000 acres.
Removing sellback limits could grow rooftop capacity much faster: pay
households and businesses the full value of all the power they export, even
when it exceeds the value of what they draw from the grid, instead of
today's limited credits. If this doubled the installation pace, rooftop
capacity would reach about 2,100 MW by 2050 and cut the utility-scale land
requirement toward 17,000 acres (Section 2.7). That trajectory is an
...
rooftop potential and ignores all parking lot and walkway canopy potential.
It shows the direction and the scale that rooftop growth substitutes for open land.
...
$0.03 billion over 2027–2050, about two-hundredths of a cent per kilowatt-hour.
Predictable behavior is nearly as good as perfect coordination. But the value 
of coordination grows with adoption. With 2.1 GW of distributed solar 
(the accelerated trajectory the sellback reform of Section 2.8 is designed to 
unlock), optimal dispatch of the rooftop batteries is worth about $0.23 billion 
(0.17 cents per kilowatt-hour) — a reason to build real-time price exposure into that reform from the start.
...
low end is their construction estimate, which excludes items such as
insurance, customs, design allowance, and contingency; the high end is
their own +20 percent sensitivity. At that midpoint the bundle costs $0.70 
billion more than building no new fuel plant at reference oil prices, 
roughly half a cent per kilowatt-hour (Appendix A.1), within a range of $0.49 
to $0.91 billion at that oil price. The gap is positive at every oil price 
tested: it widens to $1.61 billion on the market's low oil path and $1.16 
billion on its high path (Table ES.1). The more useful comparison is between 
the plant and the fuel: whatever advantage LNG offers comes from the fuel, 
not the plant. The same modern plant burning today's fuel oil costs even more, 
while LNG burned in existing plants saves money. JERA's new plant, even in 
its best case — its lowest capital estimate, with solar at twice mainland 
cost — roughly breaks even. Converting the independent Kalaeloa plant alone 
saves $0.39 billion, and converting Hawaiian Electric's Kahe 5 and 6 and 
its Campbell Industrial Park (CIP) combustion turbine as well saves nearly 
three times as much, with no new construction. The savings hold up under conservative accounting: charging Hawaiian Electric's entire 2016 multi-island
conversion budget ($450 million in today's dollars) against just the three
Oʻahu units still leaves about $0.63 billion; adding a further $260 million
of 2016 pipeline plans on top still leaves at least $0.39 billion (Sections
4.3, 4.7). Faster rooftop solar growth widens the gap against new plants because
it leaves less demand for a new plant to serve.

The two LNG questions turn out to depend on oil prices in opposite ways,
and separating them clarifies the tradeoffs. Converting existing plants is a
...
crude, that gap stays near $6.60 per million Btu whether oil is cheap or expensive
($5.30 after the regasification charge; Table 1.1). What varies across oil 
worlds is how much fuel is left to switch and how much clean investment 
cheap gas defers, so conversion savings peak on the central oil paths and 
fade at both extremes (Section 4.1) — but they never depend on the per-unit 
gap being right. Building a new plant is an efficiency decision: its value 
...
oil is cheap the plant runs hard but saves little, because the fuel 
it displaces is cheap. When oil is expensive, solar and storage are 
a better option sooner, and the plant runs at 20 percent of capacity in 
2035. So the LNG case, to the extent there is one, rests on switching 
fuel in plants Oʻahu already owns, and that case does not need an
oil-price forecast to be right. Considerations outside the cost model point
the same way. The proposal commits Oʻahu to a single supplier for two
decades: twenty years of operation for the floating import terminal (FSRU),
under supply-contract terms not yet public that are likely written to 
protect the supplier, so the quoted price is best read as a floor. It 
defers roughly a decade of clean-energy deployment, and it increases 
total greenhouse emissions: pricing the liquefaction, shipping, and 
regasification energy at the Department of Energy's own stage values 
erases the combustion advantage before any methane leakage is counted, 
and measured U.S. leakage — 2.95 percent production-weighted — pushes 
the LNG path further behind (Section 4.9). And it strands Hawaiian Electric's generation 
assets, a loss that falls on the utility's already fragile finances if 
capital recovery is disallowed, or on its customers if it is allowed 
(Sections 4 and 8).
...
projection the saving rises to about $0.9 billion; at the pessimistic
projection the model builds nothing and loses nothing, so the downside is
bounded.
...
here (Section 3) and development proceeds in partnership with Native Hawaiian 
communities (Section 3.1).
...
JERA's, builds nearly the same solar on nearly the same 20,900 acres,
differing mainly in timing. Markedly less is used if rooftop, canopy, or other
kinds of distributed solar expand meaningfully from its footprint today. The 
assumed eligible inventory exceeds what gets built, and this inventory is conservative both in quantity and quality, because most of 
the best-suited agricultural lands require a special permit, belong to the 
military, or may otherwise be challenging to develop. We built the inventory using land screens that match what Hawaiian Electric and the Hawaii State Energy Office used in developing their plans, and in a sensitivity check remove the remaining
better agricultural lands. We interpret this assumed inventory as a working
assumption, recognizing that while some of it may be unworkable in practice,
many parcels outside of it are likely viable, too.

Ultimately, the LNG question is largely separate from the land-use question: Hawaiian Electric's and the State Energy Office's (HSEO's) own studies, like ours, find that LNG does little to change the ultimate solar buildout (Sections 2.5 and 2.6) because it's the least expensive way to meet the State's RPS goal.

Next, reliability. Every scenario keeps the lights on at every modeled hour, 
across weather drawn from the 2007–2008 record (where we have the best-quality data), including its single hardest day of low sun, weak winds, and an evening peak, plus a reserve margin that can survive the largest plant falling offline. Oʻahu does not need additional thermal generation for reliability: the existing fleet, plus the planned 99 MW Puʻuloa thermal plant, suffices. Some claim a high-renewable grid still needs enough firm capacity to meet peak demand. It does not. What it needs is approximately enough firm resources to cover the highest daily average demand net of solar and wind. Ample battery storage moves energy within the day and gross demand is generally lower on low-sun, low-wind days, and even the worst days produce some solar energy (Section 5). 

Last, the mandate itself. Keeping the 2045 clean-energy requirement costs about $0.26 billion, roughly two-tenths of a cent per kilowatt-hour, because the cheapest path runs ahead of the requirement through 2040 anyway and builds 91 percent of the solar even if the requirement is removed. Abandoning it saves about $0.45 billion (about 0.3 cents per kilowatt-hour) if the replacement is a new gas plant and import terminal, and up to about $0.7 billion if existing plants are converted to LNG instead, the cheaper route Finding 3 identifies. We tested whether a new, more efficient plant can pay for itself in a no-mandate world when the model may choose freely: it declines the 2030 plant entirely and adds at most about 250 MW around 2045, as the converted units age out, for a savings that lies within the solve tolerance (Section 4.8). Either way, capturing this modest saving requires abandoning the clean-energy mandate, committing to a long-term contract, hoping (awkwardly) that geothermal and solar don't overperform, and hoping the contract price, or better, holds throughout. That last hope looks dubious: today's spot price (JKM, the Asian spot-LNG marker, July 2026) hovers around $21–22/MMBtu, nearly double the reference contract price we assume (Section 4.8).

**Also decisive.** The Waiau Repower, Hawaiian Electric's proposal to
rebuild its old Waiau oil units, raises system cost by $1.35 to $1.49 
billion at every oil price tested, and every bundle that contains it 
inherits that penalty (Section 6). (This exceeds the literal cost of 
the plant because the cost of capital is higher than our discount rate.) 
The repower's six small units carry a resiliency argument the model does not
...
than the system wants: a 375 MW version does modestly better in every configuration tested, and converting existing units beats both at JERA's quoted costs (Sections 4.1, 4.3, 4.7). One caution: JERA attributes its attractive quote partly to scale, so a smaller plant might not get the same price per megawatt.
...
the midpoint of the bare construction estimate (bare-EPC: engineering,
procurement, and construction, excluding contingency and similar items) and
the +20% capital case; the band spans the two (Figure ES.1). The conversion
row is shown at reference oil, the only oil path solved on this base-rooftop trajectory; Figure 4.3 solves the conversions across all four oil paths and three solar-cost levels on the trend-rooftop trajectory, where the saving peaks on the central paths and fades at both extremes (Section 4.1). The conversion row nets out the full 2016conversion-program estimate ($450M in 2024$) as conversion capital; its bracketed value also adds the entire 2016 onshore-pipeline package ($260M), an extreme charge by construction (Section 4.7).

![Figure ES.1 — system cost of each investment vs building no new fuel plant](figures/fig_ES1_jera_bracket.png)

*Figure ES.1 —
 Total 2027–2050 system cost of each proposed investment,
...
futures path, the EIA-anchored reference, and the market's 90th percentile
(Appendix A.14). Whiskers on the JERA bars span the plant's bare-EPC capital to +20 percent; the bar sits at the midpoint. The conversions bar (shown at reference oil; Figure 4.3 spans all four oil paths) is LNG conversion of existing
...
|---|---:|---:|---:|---:|
| No new fuel plant | 21.35 | 24.54 | 26.11 | 27.62 |
| LNG conversions, no new plant — net of conversion capital | — | — | 25.48 (−0.63) *[−0.37]* | — |
| Modern LSFO plant (250 MW) | 22.00 (+0.65) | 25.00 (+0.46) | 26.53 (+0.42) | 28.21 (+0.60) |
| JERA LNG 500 MW — midpoint | 22.96 (+1.61) | 25.16 (+0.62) | 26.81 (+0.70) | 28.78 (+1.16) |
| *…JERA band [bare-EPC, +20%]* | *[+1.40, +1.82]* | *[+0.41, +0.83]* | *[+0.49, +0.91]* | *[+0.95, +1.37]* |
| Waiau Repower only | 22.73 (+1.38) | 25.93 (+1.39) | 27.50 (+1.39) | 29.10 (+1.49) |
| Waiau + LSFO plant | 23.49 (+2.14) | 26.48 (+1.94) | 28.01 (+1.90) | 29.70 (+2.08) |
| Waiau + JERA LNG — midpoint | 24.45 (+3.10) | 26.66 (+2.12) | 28.29 (+2.19) | 30.27 (+2.65) |
...
a tradeoff the zonal grid model now under development will address ("What
we will do next").
...
LNG resellers. In March 2026 JERA terminated a 20-year supply contract with Commonwealth LNG before construction began (Section 4.6).
...
about two-fifths of total system cost in the early years at
reference oil prices, declining to about 5 percent by 2045 as renewables
displace combustion (Appendix A.4). Even if every dollar of fuel spending captured
LNG's actual price advantage over low-sulfur fuel oil (LSFO) — roughly $5
per million Btu, on a base of about $17 at reference oil prices (Table 1.1) — the 
realistic ceiling is a few percent of the bill, declining toward one percent. The 
20 percent figure is unreachable under any reading of the same data. Section 4 tests the more generous "bundled" interpretation directly, with the same conclusion.

*Table 1.1 — LSFO and LNG delivered cost at three paths of Brent, the
benchmark world oil price, 2030 (2024$).*
...
does. Section 4.7 draws out what follows.
...
from NREL's own PV-plus-battery hybrid — total 2027–2050 system cost is $26.11
billion at reference oil prices. Holding solar and battery costs 50 percent
above that baseline for the full horizon raises system cost to $28.26 billion
(+$2.1B); 70 percent above raises it to $28.98 billion (+$2.9B). The
...
billion; the LNG-versus-no-new-plant decision moves it by about $0.7
billion at its reference-oil midpoint. Solar-and-storage procurement reform
is worth several times any fuel decision in this report.
...
Benchmark, Ramasamy et al., Q1 2023, NREL/TP-87303, CAPEX component table). Bringing Oʻahu near mainland cost therefore means narrowing Hawaiʻi's excess soft cost, not removing a category every market carries. Second, the size of that excess is difficult to measure directly, because Hawaiʻi procurement records do not report these categories separately. The evidence below is correspondingly
...
could address the soft-cost gap directly by replacing the multi-stage
procurement process with transparent market access to avoided-cost pricing. The pricing and interconnection details underlying the wheeling tariff will be essential. The evidence:
...
| Utility PPA, 2024–26 vintage | ~19.5 ¢/kWh (Mahi reprice) | ~6.5 ¢/kWh | escalation mostly Hawaiʻi-specific |
...
the Hawaiʻi residential average at $2.82 with a $2.14–3.20 range. 

LBNL's *Tracking the Sun* (2024) reports much higher levels — state-level median prices of $3.2–5.2 per watt for residential systems installed in 2023. Other benchmarks generally align with its 20th percentile, because its reported prices include dealer financing fees of 5 to 50 percent on loan-financed systems that quotes exclude (LBNL, *Tracking the Sun* 2024, pp. 9, 32; vendored in sources/lbnl_tts/). Its price sample also omits systems installed with batteries, which removes nearly all of Hawaiʻi's market — virtually every new Hawaiʻi rooftop system attaches storage. We therefore use Tracking the Sun for dispersion rather than levels: its state fixed effects span roughly $2 per watt, so the roughly $1 per watt Honolulu–Phoenix gap in matched EnergySage quotes is within normal state-to-state variation. Hawaiʻi households also install smaller systems, which can hurt economies of scale. The overall evidence suggests costs comparable to other urban areas of the U.S. and a small premium over the average. 

It is worth noting that other countries with streamlined rooftop solar permitting and installation rules have much lower costs than the United States. Australia's installed unsubsidized rooftop solar costs averaged about $0.80 per watt in 2024 (AU$1.20, with all soft costs — planning, labor, permits, commissioning, and installer margin — at about AU$0.55), and costs ranged from $1.00 to $1.60 per watt in Germany at the end of 2025, half or less of what United States customers pay (IEA PVPS, *National Survey Report: Australia 2024*, Tables 9 and 11; Fraunhofer ISE, *Photovoltaics Report*, July 2026; both vendored in sources/rooftop_intl/). Studies regularly attribute all or nearly all of the difference to soft costs, including permitting, interconnection, and inspection (Seel, Barbose, and Wiser 2014, LBNL-6614E, vendored in sources/rooftop_intl/). If the State were willing to adopt such measures here and eliminate hard caps on backfeeding credit, distributed solutions might undercut grid-scale solar. And current State subsidies would not be required, saving roughly $100 million per year (Hawaiʻi Department of Taxation, *Tax Credits Claimed by Hawaiʻi Taxpayers — Tax Year 2023*: $100.1 million of renewable energy technologies credit claims; vendored in sources/dotax/).

**Utility-scale procurement.** Hawaiian Electric's Stage 1 awards (approved
2018–19) delivered four-hour solar-plus-storage on Oʻahu at 8–12 cents per
kilowatt-hour (nominal award prices; ≈10–14 cents in 2024$): Hoʻohana, Mililani
I, Waiawa Phase 1, AES West Oʻahu. The contracts approved since 2024 price the 
same service at 21–23 cents in nominal contract dollars: Mahi, a 2020-vintage 
project whose amended contract (Docket 2025-0414) roughly tripled its originally executed price, and Puʻuloa Solar, a new selection under review (a solar-plus-storage project, distinct from the 99 MW Puʻuloa engine plant of Section 4.6). Mainland prices also rose substantially over this period, roughly doubling from their 2019/20 lows: matched-configuration medians moved from about 3.1 to 6.5 cents per kilowatt-hour (LBNL *Utility-Scale Solar*, 2025 data file; levelized PPA prices  in constant 2024 dollars; Box 2.1). On the same levelized 2024-dollar basis, Hawaiʻi's matched-configuration executions of 2018–2020 carried a median near 7.7 cents per kilowatt-hour, and the amended Mahi contract is roughly 19.5 cents. Because modules, batteries, and capital are priced in global markets, the common shock added roughly the same number of cents everywhere: about 2.5 to 3.5 cents per kilowatt-hour on the mainland at matched configuration, against roughly 12 cents for the Mahi repricing of an identical project. The additional
escalation is specific to Hawaiʻi's procurement channel.

Perhaps the clearest mechanism is the interaction of fixed nominal prices with slow interconnection. A Hawaiʻi PPA fixes its price in nominal dollars at award,
...
levelized cost of utility-scale solar between 2010 and 2024, to about 4.4 
cents per kilowatt-hour, and a 93 percent decline in battery storage costs
...
economics of firm solar and wind*, 2026, vendored in sources/). Recent 
declines in battery costs have been remarkable, and largely excluded from 
consideration in this study, which anchors its assumptions on NREL's 2024 ATB 
that predates some of these declines (BloombergNEF, *2025 Lithium-Ion
Battery Price Survey*: stationary pack prices down 45 percent in 2025;
vendored in sources/bnef/).
...
interest rates were a clear fundamental driver: they raise the cost of solar and
storage by more than they raise the cost of a fuel-burning plant: a plant
with no fuel recovers almost all of its cost as capital. Real interest rates
were negative for

Hawaiʻi has, by far, the highest utility-scale solar prices in the country. 
Its grid is small, and it does not have a large interconnection backlog. Its
interconnection and procurement process is especially slow and it is difficult
to find other factors that could explain its significantly higher costs.
...
> which fold in financing, configuration, and, most important, federal
> subsidies.
>
...
> back to both sides shrinks the gap substantially. Configuration compounds
...
> above), so any estimate mixing contract vintages can mislead.
>
...
> sits well above what the Stage 1 and Kauaʻi contracts imply; the State
...
> 2023, battery costs — a disproportionately large share of a Hawaiʻi
> installation's cost — have fallen 30–45 percent, making these
> multiples somewhat conservative (IRENA, *Renewable Power Generation
> Costs in 2024*: utility-scale installed costs down 32–38 percent in
> 2024; BloombergNEF, *2025 Lithium-Ion Battery Price Survey*:
> stationary pack prices down 45 percent in 2025; both vendored in
> sources/).
>
...
> record here, floating-platform engineering, plus long interconnection lines
> owing to the deep seafloor would likely be exposed to them (Section 4.5). 
> Where a single number is needed, Hawaiʻi
> currently pays roughly double the mainland price to deploy utility-scale
> solar — and the evidence of Section 2 is that this figure mainly reflects
> processes that could be reformed.

**Land and ground rent.** Recent University of Hawaiʻi solar RFPs and
contemporary utility-scale leases price ground rent at a few tenths of a cent 
per kilowatt-hour delivered — a trivial share of project cost. If developable land
were the binding constraint, lease rates would show scarcity rents. We have not
found evidence of this yet. Nevertheless, the possibility that scarcity of land 
legally permitted to install solar could drive up rent for rare parcels is 
reason alone to liberalize land use policies. Making more land available than
necessary will facilitate competition that can keep rents reasonable. 
...
average 23–24 percent only because they are much larger — 3,800 to
4,200 MW of utility solar by 2050, depending on the rooftop trajectory —
...
times of 24–30 months for utility-scale projects. For a grid of Oʻahu's
size and topology, studies of that length are difficult to justify
technically. Yet, in practice, interconnection has taken twice as long or
more (four to seven years). And unlike the mainland, the State does not
have a huge interconnection queue to help rationalize the delay. A
connect-and-manage approach — construction proceeding on a preliminary
feasibility determination, with dispatch managed against actual conditions
— is standard practice in ERCOT and Great Britain and would shorten the
queue without weakening reliability review. Soft-cost reform lowers the
price of each project. Queue reform raises the rate at which projects
arrive, and probably lowers soft costs as well. The two are complements 
(Section 2.8).
...
connect-and-manage). Texas also carries a large interconnection queue, but
...
The Hawaiʻi premium sits in process. Hardware arrives at world prices 
plus a slight transport premium and tariff, if not produced domestically;
what Hawaiʻi adds is time, queues, and risk premia, all of which policy can
reduce (Section 2.8).

### 2.5 Does Oʻahu have enough land?

**How much land the build needs.** The model's least-cost build reaches
about 4,200 MW of utility-scale solar by 2050, with rooftop systems on the
conservative trajectory of Section 2.7 (about 1,000 MW by 2050); faster
rooftop growth cuts the utility build to about 3,800 MW on the trend
...
capacity variables — and state the judgment: probably optimistic
...
lower-capacity-factor land. At five acres per megawatt, the 2050 build needs 
about 20,900 acres (about 19,000 on the trend rooftop trajectory).² The model 
assumes tracking solar throughout, because that is
...
² *Footnote: an external benchmark for the land claim. Covelli, Virgüez,
Caldeira, and Lewis (2024, Applied Energy 375: 124054) find that a
least-cost system meeting 100 percent of Oʻahu's hourly averaged demand
from wind, solar, batteries, and hydrogen storage alone would occupy 265
km², 17.1 percent of the island, against about 85 km² (20,900 acres,
roughly 5 percent) for the least-cost build here. Their supplementary
tables locate the difference. Wind is 186 of their 265 km²: 1,026 MW of
turbines charged at 44.7 acres per megawatt, the full farm area including
spacing. The paper notes that turbine foundations occupy under 2 percent of
that area, and the rest remains available for farming or grazing. Our
framework instead caps onshore wind at 150 MW under the county setback
ordinance and meets firm needs with enhanced geothermal and existing
thermal capacity on renewable fuel. The solar blocks are nearly the same
size: 78 km² there (3,485 MW of fixed-tilt at 5.5 acres per megawatt), 85
km² here (about 4,200 MW of tracking at 5 acres per MW-AC). Their solar
factor is also dated: it comes from NREL's 2013 land-use survey (Ong et al.
2013, NREL/TP-6A20-56290) for installations under 20 MW, and the successor
study's 2019 fixed-tilt median (Bolinger and Bolinger 2022, cited above) is
about 3.6 acres per MW-AC, which would shrink their solar block by a third.
Their capacity factors are computed from horizontal irradiance (GHI/1,000
W/m²), so a tracking configuration would also reduce the megawatts of solar
and storage they need. Theirs serves all demand from utility-scale plants;
our trajectories put a further 1,000–2,120 MW on rooftops by 2050. The
paper reaches the same conclusion about substitution: shifting its mix from
wind toward solar cuts its land use by more than half at about four cents
per kilowatt-hour of added cost (their Fig. S16).*
...
B/C acreage, and B/C soil concentrates in large parcels — the largest
...
in the 27,256 eligible acres, which itself exceeds the roughly 20,900
acres the least-cost build actually uses under the most conservative rooftop
solar buildout. For perspective on the scale: in 1980 the footprint of
sugarcane and pineapple plantations was about 50,000 acres, 74% of 
it is still zoned agriculture, and 78% of the acreage the Land Study 
Bureau rated carries class A or B soils. 

Note also that at least 80% of the utility-scale solar capacity built to
date — roughly 70% counting projects accepted but later withdrawn —
does not reside on class D/E lands that make up the majority of our screen, 
Hawaiian Electric's, and HSEO's. These include projects developed on class 
A/B/C agricultural lands under special use permits, non-agricultural lands, 
and military land. These facts support the idea of the land screen being a 
conservative working assumption, not a literal characterization of what is 
likely to be built.
...
without LNG.** The JERA LNG path reaches nearly the same 2050 level as
...
renewable share runs ahead of the renewable portfolio standard (RPS)
milestones through 2040, a point
...
that band's start into the 2030s without changing the 2050 mix (Section 3.5).
...
(study pp. 6–7), and in part because its scenarios use only about a quarter of 
the land available under that cap. That cap is one choice from a menu. NREL's study 
for Hawaiian Electric (Grue et al. 2020, updated 2021, archived in this repository) reports 
...
**Where official plans project less utility solar than we do, the
difference is made up with more expensive options.** HSEO applies a larger
Hawaiʻi cost penalty to solar and none to offshore wind; at those prices
its model uses only about a quarter of the land its own screen allows. Its
Oʻahu results tables (Reference low-cost case, study p. 218; the tables run
to 2045) carry about 915 MW of utility-scale solar, about a quarter of the
3,700 MW our least-cost path builds by the same year and of what their own
land screen says is available. Its distributed-solar assumption — 1,514 MW
when offshore wind is available, 1,902 MW when it is not — is consistent
with the current trend; the pair brackets our trend rooftop trajectory and
approaches our accelerated one. The remaining gap is filled by 400 MW of
offshore wind and, from 2045, roughly 650 MW of combustion turbines burning
imported hydrogen. Our own model pivots similarly to offshore wind when
solar costs more than twice the ATB baseline while offshore wind carries no
Hawaiʻi premium (Section 4.5 examines the HSEO study's methodology
directly). Hawaiian Electric's Integrated Grid Plan similarly presents a
base scenario "integrating nearly 3,000 MW of solar and storage by 2050"
and a land-constrained scenario that shifts part of that toward offshore
wind, rooftop solar, and firm renewables. The land-constrained scenario is
the utility's plan of record (IGP Supplemental Response, Docket 2018-0088,
...
and our model selects it only when solar cost is pushed near HSEO's
...
We run three installed-capacity trajectories (A.12, A.13). The conservative one — the scenario tables' "base" rooftop trajectory —
about 1,000 MW by 2050, roughly continues the recent installation rate
in gross terms; the trend one reaches about 1,560 MW; the accelerated one,
about 2,120 MW, is an illustrative projection of the response to unlimited
sellback at avoided cost, the reform this report develops in Section 2.8,
with new installs pairing two megawatt-hours of storage per megawatt (a 6.5
kW system with one 13.5 kWh battery, already
a typical configuration). Each step of rooftop growth displaces
utility-scale build: 2050 utility solar falls from about 4,200 MW
(conservative) to 3,800 (trend) to about 3,400 (accelerated), or from
about 20,900 acres of land to roughly 17,000. Across these steps each
megawatt of added rooftop capacity displaces about 0.7 MW of
utility-scale build.
...
distributed-solar tariffs is the easiest. Between tariff reform and the
full retail wheeling of Act 266 sits a smaller step: letting a rooftop
serve the building next door on the same parcel. Jurisdictions with high
rooftop penetration are testing this now, South Australia first among
them. Concern about the land requirement 
is itself an argument for the rooftop reform: every megawatt added on a roof 
or canopy reduces the utility-scale build and the land it needs.
...
development for the next edition is designed to assess this tradeoff.
...
is $0.03 billion over 2027–2050 (about 0.02 cents per kilowatt-hour); on
...
$0.23 billion, or 0.17 cents per kilowatt-hour. All six cells are solved at
...
Two conclusions follow. At current adoption, predictable evening-shifted
...
adoption, and approaches the entire cost of the 2045 mandate (below). One 
caveat bounds these figures: they price battery scheduling only. Letting 
household consumption itself respond to prices is not modeled here,
beyond the flexible half of vehicle charging described in Appendix A.5.
Prior work finds such response is worth at least several times more on 
this one than a conventional one, so the full value of real-time pricing 
likely exceeds the battery figure.⁴
...
would redirect the same effort toward system value — likely a lighter
...
path still reaches about 3,850 MW of utility solar by 2050 — 92 percent of the
mandated build — and oil's share of grid supply falls under 5 percent
by 2050 on economics alone. What the mandate buys is pace at the end: without
it the model keeps about 22 percent oil in 2040 and 9 percent in 2045,
...
0.2 cents per kilowatt-hour — several times both cells' solve gaps
(recorded per cell in results/RESULTS_SUMMARY.csv). Achieved efficiently, the mandate is cheap insurance on pace;
...
1. **Pricing reform.** Retail wheeling under 2025 Act 266 replaces the
   ...
   directly, and it is the largest single lever in this report. Whether 
   wheeling delivers depends on getting the pricing right. The price is favorable
   ...
   competitive wholesale market, federal law (PURPA) may already
   compel, and that today reaches only installations of 100 kW or less.
2. **Procurement reform.** Several standard PPA terms add cost or delay and
   ...
   5 MW are a de facto prohibition on the projects the cap permits.
...
|---|---:|---|---:|
| Option off / none | 26.67 | 0 MW | — |
| High ($14.7M/MW gross) | ~26.67 | 0 MW | ~$0 |
| Reference ($10M/MW gross) | 26.11 | 100 MW | $0.56B |
| Low ($6.2M/MW gross) | 25.80 | 100 MW | $0.88B |

At reference cost — the base-case assumption — Enhanced Geothermal is part of
the cheapest build: the no-new-plant baseline of $26.11 billion already contains 
100 MW of it, and blocking it would raise that baseline by $0.56 billion. At the 
optimistic cost, the saving rises to $0.88 billion. At the pessimistic cost, the model builds 
...
capital cost, the high case (which builds nothing) reproduces the blocked
case, and the optimistic case is solved directly; see docs/SOLVER_NOTES.md
in the repository. 

Enhanced Geothermal also saves land. Against its no-EGS counterpart, the
solved base case's 100 MW block displaces about 480 MW of utility solar —
about 2,400 acres at five acres per MW — along with about 230 MW (1,600
MWh) of storage. And if the developable resource on Oʻahu proves larger than the
...
value also depends on solar's trajectory: EGS is worth the most in the
...
When built, the ~100 MW of flat zero-carbon baseload displaces about 480 MW of
utility solar and 230 MW (1,600 MWh) of storage in the solved base case and, in
...
Hawaiʻi's net exposure ≈ $50M / $74M; and the present value of 30 years of delivered energy at a conservative 7 cents
per kilowatt-hour covers most or all of that
...
Oʻahu's steam fleet is old and inefficient: full-load heat rates run 9,700 to 10,600 Btu per kilowatt-hour, and average
operating heat rates run near 11,000, since units cycle and spend hours part-loaded. A modern
...
**The Waiau Repower is uneconomic in every case.** It adds +$1.35 to +$1.49
...
JERA's cost range, a 375 MW version comes in $0.20 billion above no-new-plant against $0.70
billion for the 500 MW version — smaller, but still a cost
...
Configuration matters for reliability as well as price. The model builds
the 500 MW as four 125 MW units and holds spinning reserve against the
largest unit online — in line with the existing fleet, whose largest
machines are Kahe 5 and 6 at about 130 MW. The proposal does not specify
a unit configuration. A monolithic single-shaft build would put more
capacity on one contingency than the island has ever carried and raise
the reserve the rest of the system must hold; the modular assumption
here is the generous one.
...
bundle is $0.70 billion more expensive at reference oil (band +0.49 to +0.91),
$0.62 billion on the futures path, $1.61 billion at the market's low oil path,
and $1.16 billion at its high path. The U-shape has a plain cause: cheap
oil shrinks the fuel bill an efficient plant saves, and expensive oil
builds the solar and storage that displace it. Where the fuel-price
...
case, about 1.8× the mainland benchmark), the JERA plant's midpoint penalty narrows to +$0.36 billion; at the 1.7× case (about 2.0× mainland) it
narrows to +$0.21 billion, reaching break-even only at the bare-EPC capital
cost. A new LSFO plant stays more expensive
than no new plant at every solar cost, from +$0.42 billion at the baseline
...
cells are not yet solved. Differences smaller than about $0.07 billion are within the solver
tolerance of the cells they are drawn from.
...
existing plants saves $0.59 to $0.73 billion on the central paths at the
baseline solar premium, and $0.80 to $1.01 billion at the higher premiums.
At the market's high path the saving nearly vanishes at the baseline premium
(+$0.02 billion) but persists at $0.43 to $0.54 billion under the higher premiums, and at the market's low path the conversions become a small net
...
value in the bottom rows. The conversion case is therefore strongest
...
estimate that itself excludes contingency. The plant is amortized over a
15-year life ending in 2045, the same lease-to-the-mandate structure
applied to the terminal (Appendix A.8). It could run past 2045 on
renewable fuel (biodiesel, or hydrogen after conversion), but with
biodiesel prices near $33/MMBtu and many other plants available, the
residual duty is worth little.
...
vendored in `sources/`). Lazard's 2026 edition carries the combined-cycle
capital range at $1,450–2,100/kW with a 24-month build (LCOE+ v19,
assumptions p. 28, vendored in `sources/`). Our $2,900/kW basis for a
Hawaiʻi build (Section 4.3) sits above the top of that range and above
the mainland quotes. Hawaiʻi construction carries its own premium on
...
every configuration tested. What it moves is dispatch. Facing a real
part-load penalty and a higher minimum load, the optimizer runs the
plant less and buys the displaced energy from solar and storage
instead. Measured on the fleet as solved at the time of the change, LNG
imports over the horizon fell about 16 percent (293 to 246 million
MMBtu) and the plant's combustion-CO₂ advantage over the no-new-plant
path widened from about 0.5 to 1.3 Mt, tripling the leak rate at which
LNG's greenhouse edge disappears. The corrected fleet's current values
...
|---|---:|---:|
| **Existing plants only** | baseline (0) | −0.39 (Kalaeloa converted) to −1.08 (also Kahe 5 & 6, CIP CT); −0.63 net of the full 2016 conversion-program charge |
| **New 500 MW combined cycle** | +1.21 | +0.49 bare-EPC; +0.70 at the capital midpoint |

*Conversion capital is set to zero in the solved conversion cells; the −0.65
...
+$0.42 billion at 250 MW, +$0.78 billion at 375, +$1.21 billion at 500. The
...
+$0.42 billion at the baseline premium down to +$0.21 billion at the 104
...
alone it saves $0.39 billion through Kalaeloa and $1.08 billion when Kahe 5
and 6 and the CIP combustion turbine convert as well. The model even routes gas through the CIP turbine, at 11.7 MMBtu/MWh one of
the least efficient units on the island: with a fuel-price gap this large, plant
...
rest of the existing fleet instead roughly triples the saving (−1.08
...
the LSFO plant loses by $0.49 billion at matched 500 MW size and the
capital midpoint ($0.70 billion at bare-EPC). What the LSFO version buys instead is
...
sits at that modern average. Sellers concede those
...
proportional terms the gap between the fuels is widest where the
...
screen is nonetheless close to ours, and their scenarios use only
about a quarter of it for utility-scale solar under their pricing
assumptions.

**The menu.** The Puʻuloa engine plant is absent from HSEO's inventory; the no-LNG
...
point it builds 200 to 270 MW against HSEO's 400. At 1.8 times
the mainland baseline (the 1.5× sensitivity) offshore wind enters only at
the margin, about 200 MW in a few of the gas-heavy cells, and the least-cost paths
still decline it.
...
the destination: every plan reaches nearly zero fossil generation by
...
HSEO plans both lean heavily on offshore wind — 18 to 26 percent of their 2040 generation — the resource our model selects
only under the doubled solar costs discussed above. Our
...
2045 and retires it over the following decade
on economics alone. That last step is what the mandate buys, and it costs
about $0.26 billion in present value, roughly 0.2 cents per kilowatt-hour
...
total, not by type: the plans carry 257–287 MW of onshore wind, but the county setback ordinance caps onshore at about 150 MW in our framework
(Section 2.5), so offshore stands in for onshore the ordinance forbids, and
...
|---|---|---|
| IGP land-constrained (plan of record) | +$2.74B | +$1.71B |
| IGP base | +$2.19B | +$2.27B |
| HSEO oil | +$3.72B | +$2.58B |
| HSEO LNG | +$2.94B | +$1.72B |
...
*Figure 4.5 — Published plans against the least-cost path as the solar
premium rises.* Each panel is one rooftop trajectory. Each point is a
solved cell: present-value system cost (2024$ billions) against cumulative
combustion CO₂ over 2027–2050, at utility-solar premiums of 20, 80, and 104
percent over the mainland benchmark. Gray is the least-cost build on the
same settings; colors are the plan-constrained cells. The plans sit
near-vertical because their generation mixes are pinned, so higher solar
costs raise their price without changing their emissions; the least-cost
path slopes because the optimizer substitutes toward fossil generation as
solar gets dearer.

Figure 4.5 plots the tags against cumulative emissions at each solar
premium. The tags are large — roughly $1.7 to $3.6 billion, on a least-cost
base of about 24 to 29 — and they are not purchasing emissions cuts. At
...
its first buyers; we regard that as somewhere between uncertain and
...
build can spare (hydrogen-producing builds remain feasible in the model). Offered that choice, the plan cells never make
...
inherent in the contracting stage, not in any party's
conduct: before financing closes,
...
commitments (Second Amendment, August 14, 2024, SEC Exhibit 10.1; some
pricing clauses remain redacted). The system can therefore reduce oil
purchases as renewables grow; a take-or-pay LNG contract removes
that freedom. The contrast extends to term: the Par agreement runs through
January 2029 with one-year extensions, while the LNG proposal is built
around twenty years of FSRU operation.
...
8,200 MWh of bulk storage and about 2,300 MW of utility solar, and by 2050
roughly three and a half times the battery capacity of the largest completed
solar-plus-storage project in the United States (Edwards & Sanborn, 3,287
MWh). Nothing in the model prices construction logistics, port throughput,
specialized labor, or global battery supply chains. But the constraint
these numbers point to is process, not physics. Solar with storage is as
fast to build as anything on the grid: in the engineering estimates behind
EIA's own outlook, utility PV and batteries take about twelve months to
construct — only onshore wind is comparable — against 22–30 for gas
turbines and combined cycles, five years for coal, and seven for nuclear
(EIA, Capital Cost and Performance Characteristics, 2024; vendored in
sources/), and solar and storage have supplied the majority of all new U.S.
generating capacity in each of the last two years (EIA, Today in Energy,
Feb. 2025). Texas, which runs interconnection on connect-and-manage rules
(Section 2.4), grew its utility-scale solar fleet from 1.9 to more than 20
gigawatts in five years — its additions in 2024 alone were more than double
what this report's least-cost path builds by 2050. Oʻahu's slower pace
comes from the procurement and interconnection process this report
documents (Sections 2.2, 2.4, 2.8): a multi-year RFP cycle in which the
incumbent utility runs the competition and manages the queue while being
the party whose generation the entrants displace. Speed serves almost no
participant with influence over that process, and the reforms of Section
2.8 are aimed at this. The residual risks that reform cannot reach
— battery supply chains, port capacity, labor — cut against every pathway
that builds at scale (the JERA path builds most of the same solar, a decade
later), and the 1.5×/1.7× solar-cost cases price the world in which they
persist.
...
**The utility's finances.** JERA's 500 MW would join a grid already served by independent generators (the planned 99 MW Puʻuloa
engine plant, Kalaeloa, H-Power, and smaller producers),
...
gradually. What rides on that difference — the economics of Par's product slate (the
mix of fuels the refinery makes), direct jobs, in-state fuel supply, and
the biofuel joint venture — is set out in
Section 8.
...
The value of conversion is the price gap between the two fuels, times the
volume burned. Because gas is priced at about 11.8 percent of Brent and the
fuel oil Hawaiʻi buys tracks Brent at nearly the same per-unit rate, that gap stays close to $6.60 per million Btu, before the regasification
charge, across the whole range of oil prices we test (Appendix A.14). The value of a new, more efficient plant is
the fuel a better heat rate saves, which is worth more when fuel is
expensive. Section 4.1 traces how the two respond differently as oil prices
move.

One condition attaches. The near-constant fuel gap follows from oil-indexed
pricing. A contract indexed to a gas hub
...
prices. HECO's own LNG program a decade ago planned such a contract. 
...
|---|---:|---:|
| No new fuel plant, no LNG | 26.11 | — |
| FSRU + Kalaeloa conversion, no new plant | 25.72 | −0.39 |
| FSRU + Kalaeloa, Kahe 5 & 6, CIP CT conversions, no new plant | 25.03 | **−1.08** |
| *…same, net of the full 2016 conversion-program charge ($0.45B)* | *25.48* | ***−0.63*** |
| *…same, also charging the entire 2016 onshore package ($0.26B)* | *25.74* | *−0.37* |
| JERA 500 (bare-EPC), no conversions | 26.60 | +0.49 |
| JERA 500 (bare-EPC) + Kalaeloa conversion | 26.14 | +0.03 |
...
not save at all (+0.49). And the terminal needs no mandate to be used this way: 
...
large — $1,800 to $1,900 per kilowatt, both for Kalaeloa's 220 MW and for the
...
Both sit far inside the budgets. (The terminal, mooring, 
...
re-running anything, as the EGS cost sensitivity is handled: a
...
**−$0.63 billion**. The stricter bound adds the entire 2016 onshore package
on top, even
though the JERA runs already charge $460 million of import infrastructure
that covers most of the same functions; the saving is still **−$0.37
billion**. Both charges are conservative by construction: the 2016 program's
...
conversion beats building JERA's plant" rests on a margin of $0.87 billion at bare-EPC capital, about $4,000 per
kilowatt of converted capacity, which realistic conversion costs are
unlikely to consume. What remains fragile
...
Gas's 2014 global invitation shows that a competitive process for
...
|---|---:|---:|
| No new fuel plant (no gas available) | 25.85 | — |
| LNG unrestricted (model's choice) | 25.66 | −0.19 |
| JERA 500 forced — bare-EPC / +20% | 25.81 / 26.34 | −0.04 / +0.49 |

Without the mandate, the model builds 375 MW of gas capacity — offered any
size, it declines the full 500 — imports 16–21
million MMBtu of LNG per year through 2050, and the LNG advantage is $0.19
billion against the no-gas baseline. Even with the mandate gone,
*forcing* the JERA bundle does not pay: at the vendor's own bare-EPC quote it
roughly breaks even (−$0.04 billion), and at the +20 percent sensitivity it
costs $0.49 billion. Sunshine keeps most of the market either way: the
no-mandate system still builds about 2,300 MW of utility solar (55 percent
of the mandated build) with gas fully available, and 3,830 MW (92 percent) without it.

Little is at stake either way. With no gas option on
the menu, dropping the rule saves $0.26 billion (26.11 against 25.85) — about
0.20 cents per kilowatt-hour; the larger figure arises only because
abandonment also unlocks unrestricted gas: **abandoning the mandate saves about $0.45 billion over twenty-four years —
roughly three tenths of a cent per kilowatt-hour — when the replacement is
a gas plant with an import terminal (offered any size, the model builds
375 MW).** The configurations in this table exclude the conversion
of existing plants, which Section 4.7 shows is the cheaper use of LNG.
Solving the no-mandate case with conversions on the menu completes the
picture. Without the mandate, the model converts Kalaeloa, Kahe 5 and 6,
and the CIP turbine and reaches $24.37 billion; the with-mandate conversion configuration
reaches $25.04 billion, so on a like-for-like menu the mandate itself costs
about $0.67 billion, roughly half a cent per
kilowatt-hour. Offered a new plant on top of conversions, the no-mandate
model declines the 2030 proposal entirely, adding only about 250 MW around
2045 as the converted units age out, worth about $0.06 billion — within the solve tolerance. Most of the apparent gain from abandonment therefore comes
from the conversion decision, which the mandate permits: converting saves
about $1.08 billion with the requirement kept in place. These comparisons
set conversion capital to zero on both sides, so it cancels; all of them
assume LNG at the contracted price (about $11.4 per MMBtu delivered) for
the duration, while the July 2026 spot price is roughly double that.
Rooftop growth is part of why the stake shrank: distributed solar and
storage reduce the energy any new plant would serve, and the no-mandate gas
build is half what it was on a gross-load basis. For comparison, letting
solar-and-storage deployment costs persist at today's procurement-implied
level costs $2.11 billion (Section 2.1; the 1.5× sensitivity, i.e. roughly
1.8× the mainland ATB benchmark once the baseline's 20 percent Hawaiʻi
premium is included — approximately the effective level implied by the
contract prices approved since 2024, Section 2.3). Walking away from the clean-energy commitment is worth a fifth to a third
of what fixing procurement is worth.
And the ranking of levers survives the mandate's removal: with no
clean-energy requirement at all, cheaper solar deployment remains the
largest cost reducer, because solar carries most of the load in every
world. The money is in fixing procurement — with the mandate or without it.

Two readings follow. For LNG proponents: the proposal's economics improve several-fold in a
no-mandate world. The case for the proposal should state that dependence. For policy: what the mandate buys — a
fully clean grid
...
with no upstream methane — the two paths run close: 29.6 Mt for JERA
at either capital case, against 31.1 for no-new-plant over
2027–2050, so the LNG path is about 1.3–1.5 Mt lower on combustion CO₂. LNG
...
250 million MMBtu of LNG over the horizon, roughly 4.9 million tonnes of
methane throughput. Each percentage point of supply-chain leakage adds about
1.5 Mt CO₂-equivalent at a 100-year warming potential, or about 4.0 Mt at the
20-year potential — against a combustion gap of 1.3–1.5 Mt in LNG's favor. LNG's
greenhouse advantage therefore reverses at leakage above roughly one
percent (100-year basis) or a third of one percent (20-year basis). The
20-year threshold sits below every published measurement of U.S. supply
chains; the 100-year threshold sits below all but the single
lowest-leaking basin measured (Sherwin et al. 2024: Appalachia at 0.75
percent, against a production-weighted average of 2.95 percent).
...
at 1, 3, and 6 percent leakage under both warming potentials. The
ledger so far counts no liquefaction or shipping energy for LNG. That
term is far from zero: the Department of Energy's life-cycle study of
U.S. LNG exports (NETL 2019, Exhibit 6-3; vendored in sources/) puts
liquefaction, ocean transport, and regasification together at about
9.5 kg CO₂e per MMBtu delivered on a Gulf-to-Rotterdam route comparable
in distance to Hawaiʻi's. Charged to the JERA path's imports, that is
about 2.4 Mt — more than the entire combustion edge — so on the netted
ledger the plant paths fall behind the clean path before any methane
leakage is counted. The leanest conversion paths keep a 0.3–0.5 Mt
netted edge, and even their break-evens fall to 0.2–0.4 percent leakage,
below the lowest-leaking U.S. basin measured. The oil side of the ledger
has its own uncounted terms — refining energy for locally produced fuel
oil, and where that co-product of Oʻahu's refinery would burn if power
plants here stopped buying it — which pull the other way; the report
carries no default value for them, and a sourced one is invited.

---

## 5. Reliability under a renewables-dominant grid

The headline findings depend on the system meeting load every hour,
including hours when sun and wind are scarce. The model solves for the
cheapest mix that delivers reliability, timepoint by timepoint.

A grid built on solar, wind, and batteries fails differently than a grid
built on large thermal units. Traditional planning guards against the loss
of a large generator at the hour of peak demand. It sizes reserves to the
biggest unit and tracks each unit's outage rate. A renewable grid spreads
generation across thousands of small pieces: panels, inverters, battery
containers, rooftops. No single failure among them matters much. The main
risk becomes weather. Bad weather covers the whole island at once, lasts
for days, and can be seen coming days ahead. The hard hours move from the
hot afternoon to the evening and the hours around dawn. The limit that
binds becomes stored energy rather than generating capacity. Operators
manage the charge in the batteries, fill them before expected stress, and
schedule maintenance in the hours with room to spare.

The model tests both frames. It holds reserves against the largest unit
online in every hour, and it carries the system through the worst weather
day in the two-year record.

### 5.1 How reliability is tested

Reliability in the model means three things at once. Energy must balance at
every timepoint. Operating reserves must be met at every timepoint. Unit
commitment rules — minimum up and down times, ramp limits, minimum loads —
must hold on every thermal unit. A configuration that fails any test at any
timepoint is infeasible.

The reserve requirement has three parts. An upward contingency reserve
covers the full capacity of the largest unit committed in that hour. A
regulating reserve rises with solar and wind output, calibrated to the GE
Hawaiʻi RPS study and capped near a fifth of installed variable capacity. A
downward requirement is sized to 10 percent of load. Reserves come from
committed thermal units, from batteries holding the required energy, and
from renewable projects running below their potential output. In the solved
base case batteries supply most of the upward reserve and solar a small
share.

Solving every hour of 2027–2050 is computationally prohibitive, so each
investment period is represented by thirteen sample days at two-hour
resolution, following the Switch-Hawaiʻi design (Fripp 2020; Imelda, Fripp
& Roberts 2024). Twelve are chosen by K-means clustering on the 2007–2008
record. The thirteenth is the single most difficult day of that record —
November 22, 2008, low sun, weak trades, evening peak — found by
dispatching a candidate system against every hour of both years at a
$5,000/MWh penalty on unserved load, and carried at full weight in every
scenario thereafter (Appendix A.5).

### 5.2 The easy day and the hard day

The 2035 annual peak (about 1,270 MW, vehicle charging included) is
straightforward: hot
...
The model, and the demand data behind it, show otherwise. What firm
generation has to cover is
...
deficit hours. In 2035 the annual peak demand is about 1,270 MW, but the largest
daily-average net load across all sample days is about 640 MW, about
half of it, and it falls on the low-renewable November day (right panel),
where the oil fleet runs nearly flat at about 500 MW — geothermal (93 MW)
and waste-to-energy (43 MW) flat beneath it — while storage
...
lower-demand days (the hard day's peak demand, about 1,090 MW with vehicle charging, is the
lowest of the year); and even the cloudiest days still deliver meaningful
solar. This metric is conservative for Oʻahu, where storage could in principle
draw on neighboring days, which the sample-day design does not allow (Section 5.3). The build carries ≈7,800–8,200 MWh of bulk storage by
2035 and ≈11,500 MWh by 2050, nearly the same with or without a new
plant. (Storage totals are loosely pinned: they move
...
One offsetting effect appears in the demand data. 
...
### 5.3 What the test establishes, and what it leaves for v2

Every solved system serves demand and reserves through the hardest day of
the 2007–2008 record. Each sample day starts from the batteries' overnight
reserve, so no build here depends on energy banked from a better day, and
...
re-commits and re-charges around it. Demand response would add more. This report
includes none beyond the flexible half of vehicle charging (Appendix A.5),
and prior work finds such response reduces high-renewable system costs six
to twelve times more than fossil-system costs (Imelda, Fripp & Roberts
2024).

The test omits five things. Two of them understate what these systems can
do: storage cannot carry energy between days, and households do not respond
to price. Three run the other way: the weather record covers two years, and
a longer meteorological record for Oʻahu contains a sixteen-day run of low
wind; no unit is ever forced out in v1, so a storm that breaks several at
once, as in January 2024, sits outside the test; and the oil- and
biofuel-burning plants that remain depend on fuel deliveries. The three
bear most heavily on the builds with the least thermal capacity.
Section 6.3 sizes that exposure.

v2 will measure what this report bounds. It will run many weather years in
sequence, draw unit failures at random, choose maintenance timing inside
the model, and report a loss-of-load expectation for every solved build. We
are building the longer weather series it needs now. On the arithmetic in
hand, these systems likely meet conventional standards of 2 to 8 unserved
hours a year.
...
The project is approved (D&O 42411, March 2026); the live question is
scope. Two findings and one open question bear on it.

### 6.1 System cost and who pays

Forcing the repower into the build raises system cost by $1.39 billion at
reference oil ($1.35–1.49 across oil paths). The model prices the project at
...
who pays: roughly $220–310 million falls to shareholders. The plant runs at 50 percent capacity factor in 2030, falls to 4–8
percent through the late 2030s, and runs near 1 percent from 2045 as
renewables and more efficient units displace it.

### 6.2 The price, and the technology comparison

A modern combined-cycle plant on the existing fuel supply prices at roughly
$2,900/kW on this report's basis (Section 4.3), about a third below the
repower's $4,545/kW, with a full-load heat rate near 7,200 Btu/kWh against the repower's 9,900
(Table 6.1). A reviewer with long Hawaiʻi regulatory
experience pressed both halves of that comparison, and both points belong
in the record. The machines do different jobs. Simple-cycle turbines start and ramp fast
and suit peaking and contingency duty; a combined cycle suits sustained
operation. Batteries reduce the value of that speed. Storage responds
faster than any turbine, supplies contingency and regulation reserves in
the model, and every solved build carries it at scale — about 8,000 MWh
by 2035, rising to about 11,500 MWh by 2050 (Section 5.2). The model
also prices flexibility directly: ramp limits, minimum loads, minimum up
and down times, and start-stop apply to every thermal unit (Section 5.1),
so the worth of a fast fleet is in the solved costs. The role distinction
sharpens the price question. Simple-cycle
capacity is normally the cheaper machine per kilowatt: Lazard's June 2025
review carries $1,150–1,450/kW for a new-build gas peaker and $1,200–1,600
for a combined cycle (its market-quote commentary and any Hawaiʻi premium
would raise both). The repower prices simple-cycle capacity at $4,545/kW,
three to four times the peaker benchmark and above every combined-cycle
figure in the same review.
The system-cost comparison gives the same answer: no-new-plant beats the repower by
$1.39 billion, and no bundle rescues it — every Waiau-containing
configuration inherits the penalty (Table ES.1).

### 6.3 Unit size and resiliency: the open question

The strongest argument for the repower is its shape, and it stands outside
the cost comparison. Six small units spread outage risk; one large unit concentrates it. On a grid Oʻahu's size, unit size is a reliability
variable in its own right, because the largest single contingency sets the
reserve everything else must hold. The model credits part of this.
Contingency reserves scale with the largest unit online (Section 5.1), so a fleet of small units carries a smaller reserve burden than a fleet
anchored by a large one; the $1.39 billion penalty is measured with that
credit in place.

The model leaves failure itself unpriced: no unit in v1 ever suffers a
...
all 208 MW of its contracted capacity, at once (V2.md documents the record). Reviewers also
recall a neighbor-island turbine out of service for more than two years in
the 1990s, with months of rolling blackouts while it was shipped abroad for
repair — the kind of rare, long outage that many small units protect
against.

**Table 6.1 — Oʻahu's firm fleet: size, age, efficiency, and outage
rates.** Capacity, in-service year, retirement, full-load heat rate
(MMBtu/MWh), and modeled derates (scheduled/forced) are from the public
model inputs; retirements match the utility's announced deactivation
schedule, and years beyond 2050 fall outside the model horizon. Filed
EFORd is the forward-looking rate in the 2021 and 2022 Adequacy of Supply
filings (Table 3), with four-year actuals in parentheses; the filings
report Kahe 1–4 and Waiau 7–8 as a single actuals group ("grp").
Bracketed values are the IGP's August 2021 planning assumptions for units
the AOS tables omit — independent producers never appear in them. Heat
rates are full-load; average operating rates run higher (Section 4.3).
Cogeneration heat rates are net of steam credit, and the large scheduled
derates on H-Power and the refinery cogens are availability calibrations
carried from the Switch-Hawaiʻi inputs.
...
| Schofield (6 engines) | 49 | 2018 | 8 | 2048 | 8.8 | 0 / 0 | 2% |
| Kalaeloa CC (2 GT + 1 ST) | 220 | 1989–91 | 35–37 | 2055 | 8.6 | 0 / 0 | [1.5%] |
| H-Power (waste-to-energy) | 86 | 1989 | 37 | 2055 | — | 50% / 0 | [3%] |
| Refinery cogens (2) | 33 | 1982–90 | 36–44 | 2045 | ~4.7 | 12–17% / 0 | — |
| Puʻuloa engines (11 × 9) | 99 | 2027 | new | 2057 | 8.5 | 3% / 2% | — |
| *Candidate:* Waiau Repower (6 × 42) | 252 | — | — | — | 9.9 | 3% / 2% | — |
| *Candidate:* JERA CC (4 × 125) | 500 | — | — | — | 6.9 | 5% / 0 | — |
| *Candidate:* LSFO CC comparator (4 × 125) | 500 | — | — | — | 7.2 | 5% / 3.5% | — |

Table 6.1 takes stock of the fleet those filings describe. The island is
long on firm units now: about 1,350 MW of thermal capability across some
two dozen units, rising to about 1,450 MW when the Puʻuloa engines arrive
in 2027, against a peak the model puts near 1,270 MW in 2035 and a binding
daily-average net load of about 640 MW (Section 5.2). Age and outage risk
...
The modeled-derate column also states our own treatment plainly, and it is
...
inputs_nlv2b/gen_info_efor.csv, results/EFOR_PILOT.csv). No total cost moved by more
than $12 million on systems of $24 to $28 billion. Every headline
comparison reproduced on the fleet as then solved: at reference oil the
JERA band was +$0.53 to +$0.96
...
billion against $1.05. The result matches the mechanics. The fleet carries
...
turbine queue (Section 4.2). Additions started now can be online before
Waiau 7 and 8 retire in 2029. The repower arrives after them. For the next
five years, the insurance available on time is storage and solar added to
an existing fleet held in good repair.

Pricing outages would cut hardest against large single units. The model
represents the JERA plant as four 125 MW units (Table 6.1), so its largest
single contingency matches the existing fleet's largest units; what it
concentrates is the site, where four units stand behind one fuel terminal
and one interconnection, the pattern of the August 2024 Kalaeloa outage.
The same pricing would credit configurations made of many small pieces:
small thermal units, and batteries, which come in smaller pieces still and
which every build already carries.

**Sizing the tail.** The event that matters is a coincidence: several units
forced out at once, in one of the few hours with a small surplus. Both pieces can be sized from
data already in hand. In the solved no-new-plant base case, the committed
spinning stack exceeds its requirement — which itself always covers the
largest online unit plus a variability adder (Section 5.1) — by a median of about 560 MW, with hours weighted by how many days of the
year each sample day represents. The cushion falls below 50 MW in about 7
percent of hours — the weighting barely moves the share, because the thin hours land on
ordinary evenings, and rare days contribute little — and they cluster tightly: the evening ramp and the hours around
dawn, the same sliver of the day every time. Behind the committed stack stand the charged batteries and the units
available but not committed, including about 370 MW of fast-starting
turbines and engines in the 2035 fleet.

![Figure 6.1 — reserves above the requirement, hour by hour](figures/fig_reserve_cushion.png)

*Figure 6.1 — Spinning reserves above the requirement, hour by hour.* Each
point is one modeled timepoint of the no-new-plant base case, sorted from
largest cushion to smallest and weighted by the number of days its sample
day represents. The requirement covers the largest online unit plus a
variability adder (Section 5.1). The shaded band marks cushions under 50
MW: about 7 percent of hours, at the evening ramp and around dawn. Built by
`build/fig_reserve_cushion.py` from the solved outputs.

On the outage side, treat each unit as independently unavailable at its
Table 6.1 rate. In the 2035 fleet the expected capacity forced out at any
moment is about 60 MW. The chance that outages exceed one large unit (130
MW) at a given moment is about 13 percent — that is the event the reserve
requirement is built to absorb, and it absorbs it by construction.
Exceeding two large units (230 MW) has a probability near 2.4 percent, and
three (330 MW) near 0.3 percent. Those coincidences test the second layer
of defense described above. Lost load requires that layer to fail too.

Conventional standards are built around a loss-of-load expectation of
roughly 2 to 8 hours per year (North America 0.1 days, Great Britain 3
hours, Ireland 8), with Hawaiian Electric's older guideline in the middle
of this range. Assuming a plant failure rate of 10 percent, the
odds of two simultaneous failures in an hour occurring during a time of
thin reserves (7 percent) equal 0.0007, which multiplied by 8,760 hours
amounts to a little over 6 hours per year. To the extent that failures are
...
security criterion return any of the repower's $1.39 billion? And if
resiliency is the product, are six new turbines the least-cost way to buy
it, against the alternatives the model already builds — batteries, the
Puʻuloa engine plant, and existing units held as reserve?

Derating lowers a plant's modeled availability by its average outage rate.
On a small island system, where one plant can be a large share of net
demand, the average is the wrong quantity. What matters is whether several
plants are out at the same time. Simulation is the tool for that, if past
outages can be linked to the different types of weather. It draws whole
outage histories at random against years of synchronized weather and
...
largest unit out while the second largest is on maintenance (V2.md).

### 6.4 For the proceeding

Every Waiau-containing bundle is more expensive than its Waiau-free
counterpart by $1.38–1.49 billion (Table ES.1). The substantive questions
open to the parties are whether the approved scope remains the least-cost
way to meet the need it was approved to address, and whether the proceeding
remains open enough to substitute a configuration that meets that need at
lower cost. If the repower's value is resiliency, the proceeding should say
so and price it against the alternatives that provide it (Section 6.3). The
recoverable-cost gap and its incidence (Section 4.6, "utility finances")
sharpen the stakes on both sides.
...
trajectory before the 2034–36 demonstration window closes? Should the
plant, the terminal, and the fuel contract in the JERA proposal be
evaluated as separable decisions (Sections 4.7–4.8 make the case that they
are)? Where would the fuel physically come from — Gulf cargoes need a
foreign-flag carrier under a Jones Act accommodation, today a temporary
suspension, and Pacific sources such as Kitimat concentrate supply in a
few loading points? The answer probably moves neither price nor total
emissions much: LNG is fungible once loaded, world prices vary little by
port, and the climate ledger depends on the marginal source that global
demand calls forth, not the port of origin (Section 4.9). It matters for
logistics and for the State's official carbon accounting, where a
low-leakage supplier improves the reported lifecycle numbers without
changing the marginal ones. Is the Waiau scope still the least-cost answer to the need it was approved
for, and how should the resiliency value of many small units be weighed
against batteries, the Puʻuloa engine plant, and existing units held in
reserve (Section 6.3)? And how should LSFO-contract renewals be evaluated so the
current structure — a requirements contract without volume lock-in
(verified against the public contract text; Section 4.6) — is not
surrendered inadvertently?
...
schedule; Stage-4 RFP pricing against the 21–23 nominal cents per kilowatt-hour of
the
contracts approved since 2024; Act
...
**What the analysis establishes.** On system cost, under current law the
JERA proposal costs modestly more than building no new fuel plant. Taking
the midpoint of JERA's own cost range — their bare-EPC estimate and their
+20% sensitivity, which restores the customs, insurance, design-allowance
and contingency items the estimate itself says it excludes — the JERA
bundle is about $0.70 billion more expensive over twenty-four years, and
more expensive in every oil-price case tested. The bundle earns back
roughly half its up-front capital in fuel savings. In an open, competitive
market, no firm would make such an investment; regulated electricity is not
such a market. Per kilowatt-hour delivered, the difference is about half a
...
to infrastructure with one use. Its advertised price should be read as a floor: such contracts are written
to insure the supplier's return (Section 4.6). The bare-EPC cost quote in JERA's proposal carries the 
...
logistics, could not mirror that exit. None of this is priced in the model,
and all of it weighs against the same
side.
...
through 2050: LNG's efficiency and lower carbon intensity displace oil early (roughly −0.8 Mt/yr around 2030), but the plant also
displaces solar and storage that would otherwise have been built, leaving
the island's power about 25 percentage points less renewable in 2035 (about
+0.6 Mt/yr) — a decade of deferred clean energy, with the ledger closing only because the RPS
...
customers pay twice — for JERA's contract and for idle HECO plants; disallowance
...
(Section 4.6). None of these three considerations is decisive alone, but all three point
the same direction, and the cost comparison leaves no LNG saving to offset
them.
...
marker at $21/MMBtu on July 17, 2026, up more than 60 percent on the year.
And yet the deeper response ran the other way. Oil peaked near $120 a
barrel against wartime forecasts of $150–200 (Borenstein, Energy Institute,
June 2026), and global gas demand is *falling* in 2026 — by about half a
percent, the third annual decline this decade (IEA). The demand side
absorbed the shock, and much of the absorption looks permanent: Japan's
monthly imports slid from 6.2 million tonnes in January to 4.0 in May, 15
percent below a year earlier, with coal imports up 14 percent (Ministry of
Finance data via LNG Prime); China's demand fell about 4 percent March
through June; Europe's 2026 demand is down more than 2 percent — driven,
the IEA notes, by renewable expansion as much as by price; and Asia-Pacific
LNG demand is now in its second consecutive annual decline (278 → 268 → 257
million tonnes, 2024–2026), a retreat Wood Mackenzie characterizes as
"structural responses rather than purely tactical ones." The oil market
makes the same point more sharply. World oil demand is expected to fall by
about a million barrels per day this year, the first decline since 2020
(IEA, July 2026). In sixty years of data, declines like this have come only
in and around recessions: every prior year of falling world oil demand saw
world growth at or below 2.6 percent, and the deepest declines came with
growth near or below zero. The IMF expects 2026 growth near 3 percent
(Figure 8.1). Rationing without substitutes would have caused a recession,
so a decline this large in a growing economy suggests energy users are
finding alternatives. Where inventory data are public, the LNG decline
looks like reduced consumption rather than delayed purchases: Japan cut
imports while holding stocks above their five-year average. Not all of it
will stick. Europe's decline is mostly deferred buying, and Qatari volumes
return when the Strait reopens. A supply glut within a year of a Hormuz
...
fuel-price shock is driving the buyers abandoning LNG toward its
...
and, in its largest markets, shows no sign of returning. Today's high spot
prices are a war premium on a commodity in lasting retreat. That is the
context for reading a 20-year contract offer at 11.8 percent of Brent: the
terms are attractive relative to history because they are the seller's
...
shocks in a world that keeps supplying them. Hawaiʻi imports every barrel of oil it burns and would import every cargo
of LNG. Sunlight is the only major energy input produced at home. The weather-driven variability of a renewables-dominant
grid, which the model meets hour-by-hour on the hardest day in the historical
record, is by comparison a solved engineering problem. With the right political will, Hawaiʻi can likely decarbonize faster, and
more cheaply relative to its conventional generation, than any other state.
...
(3) Revisit the Waiau Repower scope to whatever extent Docket 2025-0211 allows;
it is the clearest negative-value item in the cost analysis (the resiliency
argument for its six small units is Section 6.3's open question, and
quantifying it is v2 work). (4) Fund the Enhanced Geothermal 
...
- **Source-vendoring completion**: the Lazard CCGT table, the EGS resource
  screen, the rooftop-potential derivation, the Par contract structure,
  HEI ownership shares, and the 2021–2022 Adequacy of Supply filings and
  IGP outage-assumption tables behind Table 6.1.
...
- **Onshore wind re-screened under current Honolulu rules.** The model caps
  total onshore wind at 150 MW — the existing plants (Kahuku, Kawailoa, Nā Pua Makani) plus modest headroom — a blunt stand-in for the
  county setback restrictions that have effectively halted new projects. The
  cap binds: every solved scenario builds to it. v2 will replace the cap with a
...
- **Multi-day and climate-stress reliability.** Develop synchronized wind, solar,
  ...
  adjustments. Draw unit failures at random at the rates in Table 6.1,
  choose maintenance timing inside the model, and report a loss-of-load
  expectation for every solved build (Section 5.3).
  The longer series links the high-resolution 2007–2008 record to
  NREL's NOW-23 meteorological dataset (Covelli et al. 2024 use NOW-23
  directly and count an average of 93 low-wind days a year on Oʻahu,
  with a worst run of 16 straight days).
...
2050 — has a present value of 135.6 TWh (about 198 TWh undiscounted; the six
investment periods cover 28 service years, the last running past 2050), so a $1 billion cost difference equals about 0.74 cents per
...
feasibility on the historical worst day; it does not test events outside the record ("What we will do next"). Storage state-of-charge resets between sample days, which is conservative
for the renewable-heavy paths. EV charging is partly flexible: half of each period's charging is placed by
the optimizer, half follows a fixed arrival-time schedule, and the shifting
stays within each day. The flexible half grows with adoption, from under 1
percent of served load in 2027 to about 10 percent by 2050. The model
includes no other demand response: household consumption does not respond
to prices anywhere in this report (Section 5.3).
...
capacity factor the plant delivers ≈74.5 GWh per year — $5.2M per year at a conservative 7 cents per kilowatt-hour, a 30-year
present value of $102M at the 3 percent
...
The JERA path imports ≈250 million MMBtu of LNG over 2027–2050 (model
dispatch). At ≈19.3 kg CH₄ per MMBtu, that is ≈4.9 Mt of methane throughput.
...
On the current-law base case the combustion ledger gives LNG a ≈1.3–1.5 Mt
edge (29.6 Mt JERA versus 31.1 no-new-plant), implying break-even
leakage of ≈0.9–1.0 percent (100-year basis) or ≈0.3–0.4 percent (20-year
basis). The 20-year threshold is below every measured U.S. basin; the
100-year threshold is below all but the lowest-leaking basin measured
(Appalachia, 0.75 percent), so on the marginal-source reading the
LNG path is behind on total greenhouse effect. Netting the LNG fuel
cycle moves every threshold down: NETL (2019, Exhibit 6-3) puts
liquefaction, ocean transport, and regasification at 70 kg CO₂e per MWh
generated at the study's 46.4 percent plant efficiency — 9.5 kg CO₂e per
MMBtu delivered — which charges the JERA path ≈2.4 Mt against its
≈1.3–1.5 Mt combustion edge. On the netted ledger the plant
configurations are behind at zero leakage; the optimized and
Kalaeloa-only conversion paths keep ≈0.3–0.5 Mt, with netted break-evens
of 0.2–0.4 percent (100-year), below every measured basin. Stage methane
in the NETL figure (boil-off, terminal fugitives) lies outside Sherwin's
basin measurements, so the two do not double count.
...
The edge column is combustion only; the net column charges the NETL
fuel-cycle value (9.5 kg CO₂e/MMBtu) against it, and "behind" marks
configurations whose netted edge is negative — behind the clean path at
zero leakage.

| pathway | imports (M MMBtu) | CH₄ (Mt) | edge (Mt) | net (Mt) | 100-yr | 20-yr | net 100-yr | net 20-yr |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| JERA 500 MW, bare-EPC | 251 | 4.9 | 1.3 | −1.1 | 0.90% | 0.33% | behind | behind |
| JERA 500 MW, +20% capital | 249 | 4.8 | 1.5 | −0.9 | 1.03% | 0.38% | behind | behind |
| JERA 375 MW, bare-EPC | 215 | 4.2 | 1.4 | −0.7 | 1.09% | 0.40% | behind | behind |
| JERA 375 MW, +20% capital | 216 | 4.2 | 1.3 | −0.8 | 1.04% | 0.38% | behind | behind |
| Conversion, optimized | 200 | 3.9 | 2.2 | +0.3 | 1.87% | 0.68% | 0.23% | 0.08% |
| Conversion, no new plant | 199 | 3.8 | 2.4 | +0.5 | 2.05% | 0.75% | 0.41% | 0.15% |
| Conversion, HECO configuration | 355 | 6.9 | 2.7 | −0.7 | 1.29% | 0.47% | behind | behind |
...
that comparator is therefore much larger — about 5.9 to 6.1 Mt at every solar
...
100-year basis, nearly flat across the 20, 80, and 104 percent solar
...
break-even is 2.3 percent at the study's baseline solar premium — below
the U.S. production-weighted average, so the edge does not survive
average gas there — and clears it only if solar is expensive (4.8
to 5.9 percent at the 80 and 104 percent premiums). These oil-comparator
rows are unnetted on both sides: charging LNG's fuel cycle without
charging oil's refining would bias the pairing, and the report carries
no oil-side default (Section 4.9). (Plan cells here are the
...
A unit-basis caveat applies to every capacity figure here. Neither the
permit-record series nor Hawaiian Electric's published totals states a
rating basis (DC nameplate versus AC); EIA's utility-level data, which is
AC, runs about 12 percent below the matched residential figures —
consistent with the reported series being DC nameplate at a typical
inverter loading ratio. The ambiguity does not affect the report's
projections, because every coefficient is estimated per *reported* megawatt
on the same series used to project net load — the basis cancels. It matters
only when comparing our megawatts with outside sources, which needs the
basis stated on both sides. A pre-battery calibration pins the physical
interpretation: on 2013–2019 data, when net metering put nearly all rooftop
generation on the meter and batteries were negligible, day-to-day radiation
variation identifies a grid-load reduction of 0.79 MW per reported MW at
reference irradiance (s.e. 0.03; analysis/18_prebattery_effective_mw.py) —
the expected usable AC output of a DC-nameplate megawatt after tilt,
aspect, soiling, and inverter conversion. The battery-era 0.61 additionally
nets the behind-the-meter wedge and post-net-metering tariffs; the two
bracket the tariff regimes.

Two measurement problems come first. The FERC 714 hourly series carries
clock misalignments coinciding with its reporting-format changes: 2006–2012
reads one hour early and 2021–2024 one hour late relative to 2013–2020. We
detect and correct these on hours distributed technologies cannot affect:
the pre-dawn ramp (5–7 am, sun down, batteries at overnight reserve) shifts
+0.95 hours in a single step at the 2020–2021 format boundary and stays
constant while battery capacity more than doubles — a reporting artifact,
removed by a uniform one-hour roll and validated against astronomical solar
noon (the midday net-load trough aligns within a few tenths of an hour).
Calibrating on battery-inert hours matters: detecting the shift from the
evening peak would absorb the real battery reshaping into the correction.
Second, 4 am anchors every day: its load carries almost no loading on
installed PV or batteries (+0.001 MW per MW; −0.05 MW per MWh), so
subtracting it removes demand drift (efficiency, EVs, the economy) without
touching the distributed signal.
...
a no-change forecast at short horizons and inferior at multi-year
...
then $162. The band is asymmetric around
...
barrel of Brent, so the spread between the two fuels, before the regasification charge, is
almost the same in every oil case: $6.56 per MMBtu at the market 10th
percentile in 2027 against $6.61 at the 90th. The per-unit conversion saving, from burning gas
instead of oil in existing plants, therefore barely depends on the oil
price; what moves total conversion savings across oil worlds is the volume burned and the clean investment cheap gas defers (Section
4.1). The second note is a limit on the linear link outside its estimation range.
...
which is concentrated in the years when the LNG decisions are made.
...
**Tolerance.** The plan cells are at a 0.25 percent MIP gap on the
corrected fleet, refining to 0.1 percent; their least-cost references
are at 0.1 percent. Because the quota bands leave the dispatch
some freedom, a cell's cumulative emissions are more sensitive to the
solve tolerance than its cost is; emissions figures will firm as the
plan refinements land.
...
- Berck, P., and M. Roberts (1996). "Natural Resource Prices: Will They
Ever Turn Up?" *Journal of Environmental Economics and Management* 31(1):
65–78. - Covelli, D., E. Virgüez, K. Caldeira, and N. S. Lewis (2024).
"Oahu as a case study for island electricity systems relying on wind and
solar generation instead of imported petroleum fuels." *Applied Energy*
375: 124054. doi:10.1016/j.apenergy.2024.124054.
...
*Additional peer-reviewed citations whose exact locators the authors
confirm at release:* Bernard, Kaack, and Ryan & Lidderdale (oil-price
forecast accuracy, detailed in `analysis/EIA_FORECAST_ACCURACY.md`);
Coffman et al. (2022, §A.9); Agerton (2017) and Reeve and Vigfusson (2011)
(§A.14); Bolinger and Bolinger (2022, §2.5); the Lazard LCOE+ June 2025
review, IRENA's *24/7 Renewables* (2026), and the LBNL *Tracking the Sun*,
*Utility-Scale Solar*, and *Queued Up* series (§§2.2–2.4, vendored where
noted); the A.9 employment sources (Wei, Patadia, and Kammen 2010; Rutovitz
et al. 2015 and 2025; NREL JEDI; USEER);
...
- EirGrid and SONI. *All-Island Resource Adequacy Assessment* (the all-island reliability standard of 8 hours LOLE per year).
...
- Hawaii News Now (2024). Coverage of the January 8 rolling outages and the July 31–August 2 Kalaeloa outage, Oʻahu.
...
This report is release pre-v1.032: the single-node (copper-plate) model
...
(0.1 percent optimization tolerance; a handful of hard cells stopped at
their 24-hour limit marginally above it, worst 0.16 percent, and the file
records each cell's requested and achieved gap; solve history in
docs/HARD_CELLS.md).
